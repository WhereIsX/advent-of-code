def solve_part1(puzzle)
  ...
  pawssible = lines.select do |game|
    game_result = game.split(":").last.split(";").map do |round|
      round.split(",").map do |cubes|
        c = cubes.split(" ").select { |el| el.match(/[0-9]+/) || constraints.keys.includes?(el) }
        num = c.first
        color = c.last
        # puts c, num, color
        constraints[color] >= num.to_i
      end
    end
    game_result.flatten.all?(true)
  end

  pawssible.sum do |game|
...
def solve_part2(puzzle)
  ...
  pawducts = puzzle.split("\n").map do |game|
    seen = {"red" => 0, "blue" => 0, "green" => 0}

    game.split(": ").last.split("; ").map do |round|
      round.split(",").map do |cubes|
        c = cubes.split(" ").select { |el| el.match(/[0-9]+/) || constraints.keys.includes?(el) }
        num = c.first.to_i
        color = c.last
        # puts c, seen
        m = [seen[color], num].max
        seen[color] = m
        # puts seen
      end
    end
