...
describe ChronalCoordinates do
  ...
  let(:sample_origins) {ChronalCoordinates.new('1, 1
    1, 6
    8, 3
    3, 4
    5, 5
  ...
  it 'draws the origins on the grid' do
    ...
    expect(sample_origins.draw_origins).to eq(drawn_origins)
  end
  ...
  it 'fills in the grid' do
    ...
    expect(sample_origins.fill_in_grid).to eq(filled_in_grid)
  end

  it 'determines that D has an area of 9' do
    expect(sample_origins.area('D').to eq(9))
  end

  it 'determines that E has an area of 17' do
    expect(sample_origins.area('E').to eq(17))
  end

  it 'determines that A has "infinite" area' do
    expect(sample_origins.area('A').to eq('infinite'))
  end

  it 'understands that A, B, C and F have "infinite" area' do
    expect(sample_origins.coordinates_with_infinite).to eq(['A', 'B', 'C', 'F'])
  end

  it 'determines the largest finite area is 17' do
    expect(sample_origins.largest_finite_area).to eq(17)
  end
end
