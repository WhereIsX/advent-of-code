...
def parse_part_2(puzzle)
  ...
  cube_world = Hash(Tuple(Int32, Int32, Int32, Int32), Char).new 
  
  padded_puzzle.each_with_index do |row, y|
    # row == "..#.."
    row.each_char_with_index do |cube, x|
      cube_world[{x, y, 0, 0}] = cube
      [-1, 0, 1].each_repeated_permutation(2) do |(z, w)|
        if !cube_world.has_key?({x, y, z, w})
          cube_world[{x, y, z, w}] = '.'
        end 
      end 
    end 
...
{active_cube} => [
  inactive_neighborA, inactive_neighborA, inactive_neighborA => 
  make neighborA active ]
